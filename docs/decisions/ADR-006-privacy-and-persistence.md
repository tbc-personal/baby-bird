...
## Decision
- State (`method`, `inputDate`, `settings`) lives in `localStorage` under a versioned key (`puffinbaby.v1`). No cookies, no analytics, no error reporting service, no third-party scripts except the Macaulay Library embed frames (ADR-003).
