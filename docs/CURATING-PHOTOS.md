...
That writes `docs/research/commons-shortlist-14-22.md` and caches the survey
JSON beside it. Neither is committed — they are working sheets, regenerated by
the command above whenever you need them. Open the sheet in any Markdown
preview that fetches remote images, so the thumbnails render, and pick a letter
per week.
