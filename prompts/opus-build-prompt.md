...
3. `docs/decisions/ADR-001` through `ADR-006`
...
## Deliverables (all seven milestones in `docs/PLAN.md` §4)
...
### M2 Setup and Today
- Setup screen per mockup 1. Include the dashed third option "Due date from ultrasound" but behind a feature flag `ENABLE_ULTRASOUND_METHOD` defaulting to true; the author may veto.
- Storage per ADR-006, including the `?m=&d=` share link and the "replace saved date?" confirmation.
- Today screen per mockup 2, using `ComparisonCard`. Unit toggle (imperial default; metric conversions rounded sensibly: cm to 1 decimal, g to whole).
- Empty, too-early, week-3-gap, and past-42 states from the mockup's state table.

### M3 Images
- `MacaulayEmbed` component: builds the iframe from a single template constant and `mlAssetId`. Lazy-load (IntersectionObserver). Timeout of 6 s or `offline` → render `Silhouette` with the "Photo needs a connection" tag. Verify the real embed `src` pattern from a live Macaulay Library asset page's Embed dialog before hard-coding it; record what you found in a code comment and in `docs/decisions/ADR-003-images.md` under "Embed mechanics".
...
- Labor panel per mockup 4: card on Today from 34w0d, detail screen at `#/labor`, daily curve as inline SVG drawn from the model, caveat text and sources, and a "Hide this panel" toggle stored in settings.
...
- About screen: sources, credits (Macaulay Library, Wikimedia Commons contributors, Datayze as the source of the size table the author used, the studies behind the labor model), non-commercial statement, "not medical advice", "Forget my data".
...
- Dark theme support via `prefers-color-scheme`, using the token palette from the mockup CSS (`--ground`, `--paper`, `--ink`, `--ink-2`, `--line`, `--teal`, `--teal-soft`, `--orange`, `--yellow`, `--brown`, `--egg`).
- Fonts: Bricolage Grotesque (display), Atkinson Hyperlegible (body), IBM Plex Mono (numbers), self-hosted in `public/fonts/` with proper fallback stacks, not loaded from Google at runtime (the app should work offline).
